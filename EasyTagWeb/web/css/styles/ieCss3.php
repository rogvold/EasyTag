<?php
/**
 * @package WordPress
 * @subpackage Riviera
 * @since Riviera 1.0
 * 
 * CSS 3 Rules for IE < 9
 * Created by CMSMasters
 * 
 */


header('Content-type: text/css');


require('../../../../wp-load.php');

?>

.dropcap2,
#slide_top,
body.cmsms_black code, 
.wrap_nav,
.wrap_nav.navi_scrolled, 
body.cmsms_black .wrap_nav.navi_scrolled, 
.wrap_nav nav, 
body.cmsms_black .wrap_nav nav, 
#header, 
body.cmsms_black #header, 
.headline, 
body.cmsms_black .headline, 
.cmsms_breadcrumbs, 
.middle_inner,
#top, 
body.cmsms_black .middle_inner, 
body.cmsms_black #bottom, 
body.cmsms_black .table thead th,
body.cmsms_black .table tfoot th, 
body.cmsms_black .table tbody td, 
body.cmsms_black span.dropcap2, 
body.cmsms_black .success_box, 
body.cmsms_black .error_box, 
body.cmsms_black .warning_box, 
body.cmsms_black .download_box, 
body.cmsms_black .notice_box, 
body.cmsms_black .css_box, 
body.cmsms_black .featured_block, 
body.cmsms_black div.jp-playlist li, 
body.cmsms_black div.jp-playlist li.jp-playlist-current a, 
body.cmsms_black .cmsms_pricing_table, 
body.cmsms_black .percent_item, 
body.cmsms_black .post.format-aside .cmsms_post_right_column,
body.cmsms_black .archive .project.format-aside .cmsms_post_right_column,
body.cmsms_black .post.format-quote .cmsms_post_right_column,
body.cmsms_black .archive .project.format-quote .cmsms_post_right_column, 
body.cmsms_black .about_author_box, 
body.cmsms_black .comment-body, 
body.cmsms_black .pj_filter_container ul.pj_filter_list, 
body.cmsms_black #bottom .tab.lpr .tabs li a,
body.cmsms_black #bottom .tab.lpr .tabs.active li:first-child a, 
body.cmsms_black #bottom .tab.lpr .tabs li:first-child a,
body.cmsms_black #bottom .tab.lpr .tabs.active li.current a,
body.cmsms_black #bottom .tab.lpr .tabs.active li.current:first-child a,
body.cmsms_black #bottom .tab.lpr .tabs.active li:first-child a:hover, 
body.cmsms_black #bottom .tab.lpr .tabs li.current a,
body.cmsms_black #bottom .tab.lpr .tabs li a:hover, 
body.cmsms_black #bottom .tab.lpr .tab_content, 
body.cmsms_black input[type="text"], 
body.cmsms_black input[type="password"], 
body.cmsms_black textarea, 
body.cmsms_black select, 
body.cmsms_black #bottom input[type="text"], 
body.cmsms_black #bottom input[type="password"], 
body.cmsms_black #bottom textarea, 
body.cmsms_black #bottom select, 
body.cmsms_black input[type="text"]:focus, 
body.cmsms_black input[type="password"]:focus, 
body.cmsms_black textarea:focus,
body.cmsms_black select:focus,
body.cmsms_black option,
body.cmsms_black #bottom input[type="text"]:focus, 
body.cmsms_black #bottom input[type="password"]:focus, 
body.cmsms_black #bottom textarea:focus,
body.cmsms_black #bottom select:focus,
body.cmsms_black #bottom option 
.cmsmsLike {behavior:url(<?php echo get_template_directory_uri(); ?>/css/pie.htc);}

.wrap_nav.navi_scrolled {-pie-background:rgba(255, 255, 255, .9);}

body.cmsms_black .wrap_nav.navi_scrolled {-pie-background:rgba(0, 0, 0, .9);}

.wrap_nav nav {-pie-background:rgba(255, 255, 255, .5);}

body.cmsms_black .wrap_nav nav {-pie-background:rgba(0, 0, 0, .4);}

#header,
.headline,
.cmsms_breadcrumbs,
.middle_inner,
#top {-pie-background:rgba(255, 255, 255, .8);}

body.cmsms_black #header, 
body.cmsms_black .headline,
body.cmsms_black .cmsms_breadcrumbs,
body.cmsms_black .middle_inner,
body.cmsms_black #top, 
body.cmsms_black .table thead th,
body.cmsms_black .table tfoot th,
body.cmsms_black span.dropcap2,
body.cmsms_black .featured_block,
body.cmsms_black .cmsms_pricing_table,
body.cmsms_black .post.format-aside .cmsms_post_right_column,
body.cmsms_black .archive .project.format-aside .cmsms_post_right_column,
body.cmsms_black .post.format-quote .cmsms_post_right_column,
body.cmsms_black .archive .project.format-quote .cmsms_post_right_column,
body.cmsms_black .about_author_box,
body.cmsms_black .comment-body,
body.cmsms_black .pj_filter_container ul.pj_filter_list {-pie-background:rgba(0, 0, 0, .6);}

body.cmsms_black #bottom {-pie-background:rgba(11, 11, 11, .8);}

body.cmsms_black .table tbody td {-pie-background:rgba(255, 255, 255, .07);}

body.cmsms_black .success_box {-pie-background:rgba(195, 228, 80, .15);}

body.cmsms_black .error_box {-pie-background:rgba(245, 157, 98, .15);}

body.cmsms_black .warning_box {-pie-background:rgba(237, 171, 13, .15);}

body.cmsms_black .download_box {-pie-background:rgba(233, 246, 92, .15);}

body.cmsms_black .notice_box {-pie-background:rgba(142, 179, 242, .15);}

body.cmsms_black .css_box {-pie-background:rgba(241, 214, 43, .15);}

body.cmsms_black div.jp-playlist li {-pie-background:rgba(0, 0, 0, .2);}

body.cmsms_black div.jp-playlist li.jp-playlist-current a,
body.cmsms_black .percent_item {-pie-background:rgba(255, 255, 255, .1);}

body.cmsms_black #bottom .tab.lpr .tabs li a,
body.cmsms_black #bottom .tab.lpr .tabs.active li:first-child a {-pie-background:rgba(0, 0, 0, .3);}

body.cmsms_black #bottom .tab.lpr .tabs li:first-child a,
body.cmsms_black #bottom .tab.lpr .tabs.active li.current a,
body.cmsms_black #bottom .tab.lpr .tabs.active li.current:first-child a,
body.cmsms_black #bottom .tab.lpr .tabs.active li:first-child a:hover, 
body.cmsms_black #bottom .tab.lpr .tabs li.current a,
body.cmsms_black #bottom .tab.lpr .tabs li a:hover,
body.cmsms_black #bottom .tab.lpr .tab_content {-pie-background:rgba(0, 0, 0, .7);}

body.cmsms_black input[type="text"], 
body.cmsms_black input[type="password"], 
body.cmsms_black textarea, 
body.cmsms_black select, 
body.cmsms_black #bottom input[type="text"], 
body.cmsms_black #bottom input[type="password"], 
body.cmsms_black #bottom textarea, 
body.cmsms_black #bottom select {-pie-background:rgba(64, 64, 64, .14);}

body.cmsms_black input[type="text"]:focus, 
body.cmsms_black input[type="password"]:focus, 
body.cmsms_black textarea:focus,
body.cmsms_black select:focus,
body.cmsms_black option {-pie-background:rgba(15, 15, 15, .3);}

body.cmsms_black #bottom input[type="text"]:focus, 
body.cmsms_black #bottom input[type="password"]:focus, 
body.cmsms_black #bottom textarea:focus,
body.cmsms_black #bottom select:focus,
body.cmsms_black #bottom option {-pie-background:rgba(255, 255, 255, .07);}







